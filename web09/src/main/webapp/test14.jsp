...
<%
	...
	//2. apach-commons-fileupload 라이브러리를 이용하여 파일 업로드 및 저장을 수행
	//- 장점 : 같은 이름으로 다중 선택되어 전송되는 파일을 처리할 수 있다.
	//- 단점 : 코드가 상대적으로 복잡하다.
	
	//cos.jar에서는 MultipartRequest라는 클래스를 제공하며 이를 통해 multipart 요청을 처리
	//= multipart/form-data 방식에서는 99.999% 확률로 "파일이" 전송된다.
	//= 따라서 파일의 처리와 관련된 값들을 설정해야 요청을 수신할 수 있다.
	//= (1) 해석할 요청 객체 
	//= (2) 저장될 위치
	//= (3) 허용할 업로드 제한크기(byte)
	//= (4) 해석할 Encoding 방식
