...
@WebFilter(urlPatterns = { "/board/edit.jsp", "/board/edit.kh", "/board/delete.kh" })
public class BoardSelfFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		/**
		 * 시나리오 1. 모든 요청에는 boardNo라는 파라미터가 존재한다(만약에 없으면 차단) 2. boardNo를 이용해서 해당 번호 게시글의
		 * "작성자(boardWriter)"를 조회한다. 3. 세션을 조회하여 현재 로그인한 사용자의 ID를 알아낸다. 4. 2번과 3번에서 알아낸
		 * 정보를 비교한다. 5-1. 4번의 결과가 같다고 나올 경우 본인 글이므로 통과시킨다. 5-2. 4번의 결과가 다르다고 나올 경우 본인 글이
		 * 아니므로 차단시킨다.
		 */
		...
		try {
			...
			String memberId = (String) req.getSession().getAttribute("logkey");
