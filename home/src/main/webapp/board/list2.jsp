...
%>
<br>
startBlock = <%=startBlock%>, finish = <%=finishBlock%>,count = <%=count %>,
lastBlock = <%=lastBlock %>
...
<%
	...
	if(search) {//검색
			...
			list = boardDao.searchByTopN(column, keyword, begin, end);//원하는 구간 검색결과 구간
	}
	else {
		//list = boardDao.select();//모든목록
		list = boardDao.listByTopN(begin, end);//원하는 구간 목록
	}
%>
...
<!-- 검색창 -->
	...
	<select name="column">
		<%if(column != null && column.equals("board_no")){ %>
		...
		<%if(column != null && column.equals("board_time")){ %>
